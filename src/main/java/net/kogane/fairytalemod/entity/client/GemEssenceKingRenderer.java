...
public class GemEssenceKingRenderer extends MobRenderer<GemEssenceKingEntity, GemEssenceKingModel<GemEssenceKingEntity>> {
    private static final ResourceLocation GEM_ESSENCE_KING_LOCATION = new ResourceLocation(FairyTaleMod.MOD_ID,"textures/entity/gem_essence_king.png");

    public GemEssenceKingRenderer(EntityRendererProvider.Context pContext) {
        super(pContext, new GemEssenceKingModel<>(pContext.bakeLayer(ModModelLayers.GEM_ESSENCE_KING_LAYER)), 0.9f);
    }
    ...
    public void render(GemEssenceKingEntity pEntity, float pEntityYaw, float pPartialTicks,
        ...
        if(pEntity.isBaby()) {
            pMatrixStack.scale(3.45f, 3.45f, 3.45f);
        }
